...
const CreateKeyboard = (lang, shift) => {
  ...
  for (let i = 0; i < lang.length; i += 1) {
    ...
    key.classList = 'key';
    key.id = codes[i];
    if (Array.isArray(lang[i])) key.innerText = lang[i][shift];
    else key.innerText = lang[i];
...
const ChangeCase = (keysLang1, switchTo) => {
  if (keysLang1 === 'ru') CreateKeyboard(keysRu, switchTo);
  else CreateKeyboard(keysEn, switchTo);
};

const ChangeLang = (keysLang1) => {
  if (keysLang1 === 'ru') CreateKeyboard(keysEn, 0);
  else CreateKeyboard(keysRu, 0);
};
...
const KeyDown = (event) => {
  if (codes.indexOf(event.code) !== -1) {
    ...
    if (event.code !== 'CapsLock') downKey.classList.add('key-pressed');
  }

  pressed.add(event.code);
  if (pressed.has('AltLeft') && pressed.has('ShiftLeft')) ChangeLang(keysLang);
};
...
const KeyUp = (event) => {
  if (codes.indexOf(event.code) !== -1) {
    ...
    if (event.code === 'ShiftLeft' || event.code === 'ShiftRight') ChangeCase(keysLang, 0);
...
const MouseUp = (event) => {
  if (event.target.classList.contains('key')) {
    if (event.target.id === 'ShiftLeft' || event.target.id === 'ShiftRight') ChangeCase(keysLang, 0);
    else if (event.target.id === 'CapsLock') return;
...
CreateBaseHTML();
CreateKeyboard(keysLang === 'ru' ? keysRu : keysEn, 0);
